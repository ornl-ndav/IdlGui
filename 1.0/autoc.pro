pro autoc,simb,n,resres,thre=thre,hkl=hkl,a=a,q=q,nonman=nonman,ndeg=ndeg,na=na,dq=dq,excl=excl,maxq=maxq
...
tottube=findgen(8)
for i=0,7 do tottube(i)=total(data(i*128l:i*128+127,*))
m=median(tottube)
...
nq=s(2)
tubes=fltarr(8,nq)
for i=0,nq-1 do begin&for j=0,7 do begin&tubes(j,i)=total(data(j*128l:j*128+127,i))&end&end

temp2=fltarr(20)
result=fltarr(128,8)
for i=excl,127-excl-1 do begin&$
for j=0,7 do begin&$
for k=0,19 do begin&temp1=shift(data(i+j*128,*),k-10)&temp2(k)=total(data(i+1+j*128,*)*temp1)&end&$
res=poly_fit(findgen(20)-10,temp2,2)&$
result(i,j)=-res(1)/2./res(2)&$
end&end
stop
