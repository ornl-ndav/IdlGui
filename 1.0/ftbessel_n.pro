pro ftbessel_n,q,int,r,ft,lorch=lorch,n_bessel=n_bessel

; calculates the n th order bessel transform
; FT of int(Q)  -> ft(r) the meaning of Q and r may be reversed ; actually it calculates  sum i(Q) sin(Qr)/(Qr) Q^2 dQ

if (not var_defined(n_bessel)) then n_bessel=0
...
if (lorch eq 0) then begin&for i=0,nr-1 do begin&x=q*r(i)& ft(i)=total(int1*sp_beselj(x,n_bessel)) &end&end

if (lorch ne 0) then begin&for i=0,nr-1 do begin&x=q*r(i)& ft(i)=total(int1*sp_beselj(x,n_bessel)*j0(!pi*q/max(q))) &end&end
