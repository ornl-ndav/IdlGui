; $Id: //depot/idl/releases/IDL_80/idldir/lib/utilities/xloadct.pro#1 $
;
...
if event.id eq state.name_list then begin
	...
	widget_control, event.top, get_uvalue=global_xloadct
	global = (*global_xloadct).global
	
	LOADCT, silent=silent, event.index, FILE=filename, NCOLORS=ncolors, $
		BOTTOM=cbot
		(*global).default_colorbar = event.index
	goto, set_gamma
	ENDIF
...
silent = (N_ELEMENTS(SILENT_f) gt 0) ? KEYWORD_SET(SILENT_f) : 1
...
global_xloadct = ptr_new({ global: global})
widget_control, base, set_uvalue=global_xloadct
