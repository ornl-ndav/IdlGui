...
path = '~/IDLWorkspace80/SNS_offspec/'
...
shade_surf, smooth(thlam.data,3), thlam.lambda, thlam.theta, ax=70, charsi=1.5, xtitle='LAMBDA (' + string(30*4b) + ')', ytitle='THETA (rad)'
wait,.1
wshow
...
;xsi=500
;ysi=500
;window,/free,xsize=xsi, ysize=ysi,title='Scaling Data...', xpos=500, ypos=60
;wset1=!d.window
;window,/free,/pixmap,xsize=xsi, ysize=ysi,title='pixmap'
;pixmapID=!d.window

;for loop=1,num-1 do begin
;
;    wset, pixmapid
;    plot, QZvec, specular[loop-1,*],/ylog, psym=1, charsi=1.5, xtitle='QZ', ytitle='R'
;    oplot, QZvec, specular[loop,*], psym=1, color=200
;
;    wset, wset1
;
;    wait,1
;    bnp = 0
;    while bnp eq 0 do BEGIN;button not pressed
;        device,copy=[0,0,xsi,ysi,0,0,pixmapID]
;        cursor,qx1,y1,/data,/nowait
;        cursor,x1,y1,/device,/nowait
;        if !mouse.button eq 1 and x1 ne -1 and y1 ne -1 then bnp =1
;        plots, [x1,x1],[0,ysi],/device
;        XYOUTS,x1,150,qx1,/Device,SIZE=2
;    endwhile
;
;    wset, pixmapID
;    plots, [x1,x1],[0,ysi],/device
...
;    while bnp eq 0 do BEGIN;button not pressed
;        device,copy=[0,0,xsi,ysi,0,0,pixmapID]
;        cursor,qx2,y1,/data,/nowait
;        cursor,x2,y1,/device,/nowait
;        if !mouse.button eq 1 and x1 ne -1 and y1 ne -1 then bnp =1
;        plots, [x2,x2],[0,ysi],/device
;        XYOUTS,x2,100,qx2,/Device,SIZE=2
;    endwhile
;
;    Qlo=qx1<qx2
;    Qhi=qx1>qx2
;
;    lo=min(where(QZvec ge Qlo))
;    hi=max(where(QZvec le Qhi))
;
;   print, Qlo, Qhi, lo, hi
;
;    list=where(QZvec ge Qlo and QZvec le Qhi)
;    si=size(list,/dim)
;    si=si[0]
;
;    ratio=specular[loop-1,list]/specular[loop,list]
;
;    print, total(ratio)/si
;    scale[loop]=((total(ratio)/si))*scale[loop-1]
;
;    ;ZERO OUT DATA OF LOOP-1 AFTER QHI
;    specular[loop-1,where(QZvec gt Qhi)]=0
;
;    ;ZERO OUT DATA OF LOOP BELOW QLO
;    specular[loop,where(QZvec lt Qlo)]=0
;
;endfor
;
;wdelete,wset1
;
;plot, QZvec, specular[0,*]*scale[0],/ylog, psym=1, charsi=1.5, xtitle='QZ', ytitle='R'
;for loop=1,num-1 do begin
;    oplot, QZvec, specular[loop,*]*scale[loop], psym=1
;endfor
;wshow
;scalen=scale/scale[0]
...
;
;
;       pos2=max(where(specular[loop,*] ne 0))-step
;       pos1=min(where(specular[loop,*] ne 0))+step
;       trim[loop,pos1:pos2]=specular[loop,pos1:pos2]
;
...
;       help, overlap
;
;       pos2=max(where(trim[loop-1,*] ne 0))-step
;       pos1=min(where(trim[loop,*] ne 0))+step
;        ;tot1=total(trim[loop-1,pos1:pos2])
;        ;tot2=total(trim[loop, pos1:pos2])
;        ;scale[loop]=tot1/tot2
;
;        ratio=trim[loop-1,pos1:pos2]/trim[loop,pos1:pos2]
;        scale[loop]=((total(ratio)/(pos2-pos1+1)))*scale[loop-1]
;    endif
;
;    ;plot em up
;    print, loop,n_elements(where(specular[loop,*] ne 0))
;    oplot, QZvec, specular[loop,*]*scale[loop]
;    oplot, QZvec, trim[loop,*]*scale[loop], color=200
;
;    wait,.1
;endfor
