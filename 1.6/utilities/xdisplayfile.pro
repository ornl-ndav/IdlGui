; $Id: //depot/idl/IDL_71/idldir/lib/utilities/xdisplayfile.pro#1 $
;
  ...
  CASE retval OF
    ...
    "EXIT": BEGIN
      ...
      IF (WIDGET_INFO(state.ourGroup, /VALID)) THEN $
        ...
        event=state.event
        activate_button, event, 'preview_command_line', 1
    END
    ELSE:
  ENDCASE
END
...
PRO XDisplayFile, FILENAME, TITLE = TITLE, GROUP = GROUP, WIDTH = WIDTH, $
  ...
  IF(KEYWORD_SET(CENTER)) THEN BEGIN
    center_flag = 1b
    IF (NOT(KEYWORD_SET(GROUP))) THEN center_flag = 0b
  ENDIF
  ...
  if KEYWORD_SET(MODAL) then begin
    ...
    if (center_flag) then begin
    
      main_base_geometry = WIDGET_INFO(group,/GEOMETRY)
      main_xoff = main_base_geometry.xoffset
      main_yoff = main_base_geometry.yoffset
      main_xsize = main_base_geometry.xsize
      main_ysize = main_base_geometry.ysize
      
      xoff = ((main_xsize) / 3) + main_xoff
