...
;                        except complex, double complex, pointer or object. Must be
;                        a scalar.
;
; KEYWORDS:
;
...
;     Small changes to do all calculations in DOUBLE and LONG64. 22 February 2007.
;     DWF.
;     Made it possible to pass a vector of numbers to the program. 18 August 2007.
;     DWF.
;-
;###########################################################################
;
...
FUNCTION number_formatter, number, DECIMALS=decimals
   ...
   IF N_Elements(number) EQ 0 THEN $
     Message, 'A number must be passed as an argument to the function.'
   IF N_Elements(decimals) EQ 0 THEN decimals = 2

   ; If the number is a byte, convert it to an integer and return it.
   IF Size(number[0], /TNAME) EQ 'BYTE' THEN RETURN, $
     StrTrim(String(Fix(number), Format='(I3)'),2)
...
; Is the number a negative value? Deal only with positive values, until the end.
      ...
      CASE Size(theNumber, /TNAME) OF
         ...
         'FLOAT': BEGIN
            ...
            CASE loc OF
               ...
               -1: BEGIN ; No exponent.
                   ...
                   fracpart = StrTrim(Round(Double('1.' + parts[1]) $
                                            * (10L^decimals), /L64), 2)
               ...
               ELSE: BEGIN
                   ...
                   ; Round to the number of decimals you want.
                   fracpart = StrTrim(Round(Double('1.' + p[0]) $
                                            * (10L^decimals), /L64), 2)
                   IF StrMid(fracpart,0,1) EQ '2' THEN BEGIN
                     parts[0] = StrTrim(Long64(parts[0]) + 1, 2)
                   ENDIF
                   ...
                   retValue[j] = StrTrim(parts[0],2) + '.' + p[0] $
                     + StrLowCase(StrMid(parts[1],loc,1)) + expSign + expPart
                   END
         ...
         ELSE: Message, 'Cannot format a number of this type: ' $
           + Size(theNumber, /TNAME) + '.'
