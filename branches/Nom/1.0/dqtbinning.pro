...
if norm_beam then natatime=d1(wn1(i));*2
t1=systime(1)
readu,1,data
t2=systime(1)
w=where(data(lindgen(Natatime)*2)-shift(data(lindgen(natatime)*2),-1) gt 160000,n1)
w1=where(data(lindgen(Natatime)*2+1) ge 2^30.,nn1)
if nn1 gt 0 then data(w1*2+1)=0
laseron=0
...
;	if norm_beam then h=h*normf(w1(i))
	...
	for kk=0,n1-1 do histo(j,d(kk))=histo(j,d(kk))+normf(wn1(i))
...
;if n2 gt 0 and n_elements(d) gt 1 then begin&print,i,j,d&stop&end
		...
		d=d(w1)&$
		histo(j,d)=histo(j,d)+normf(wn1(i))&$
;		u=uniq(d,sort(d))&$
