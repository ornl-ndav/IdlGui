 Pro xrayf,q,e,formel,formel1,iso,modi,iso0,incoh,fin
...
;          vorhanden sind, z. B: C2H5NO: formel=[6,1,7,8], formel1=[2,5,1,1]
;
; Output :
...
f=dblarr(nq1,nats)
s=dblarr(nq1,nats)
; f:Die elastischen Formfaktoren an den Stuetzstellen
; s: Comptonstreuamplitude
readf,1,f
...
sumf=dblarr(nq)
iso=dblarr(nq)
for i=0,nq-1 do sumf(i)=total(fin(i,*)*formel1)
for i=0,nq-1 do iso(i)=total(fin(i,*)^2*formel1(*))
modi=1./sumf^2
...
sums=dblarr(nq)
for i=0,nq-1 do sums(i)=total(sint(i,*)*formel1(*))
        aB=e/MEEV
